/*!
 * Source adapted from https://github.com/nepsilon/search-query-parser
...
function parse(string, options) {
	...
	// Otherwise parse the advanced query syntax
			...
			if (sepIndex !== -1) {
				...
				if (isExcludedTerm) {
					if (exclusion['content']) {
						if (exclusion['content'] instanceof Array) {
							exclusion['content'].push(term);
						} else {
							exclusion['content'] = [exclusion['content']];
							exclusion['content'].push(term);
						}
					} else {
						// First time seeing an excluded content term
						exclusion['content'] = term;
					}
				} else {
					terms.push({
						content: term,
		...
		while (term = terms.pop()) {
			...
			if (term.content) {
				// We add it as pure content
				query.content.push(term.content);
			...
			// We got an advanced search syntax
				...
				// The key allows a range
					...
					// We add it as pure content
					var content = term.keyword + ':' + term.value;
					query.content.push(content);
		...
		// Concatenate all content terms if any
		if (query.content.length) {
			if (!options.tokenize) {
				query.content = query.content.join(' ').trim();
			}
		}
		// Just remove the attribute content when it's empty
		else {
